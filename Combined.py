...
graph_opts = dict(cmap = 'RdBu_r', symmetric=True, logy = True, colorbar = True, \
                ylim=(1000,0.1), active_tools=['pan'],title='Tap to compare ens')
...
@pn.depends(stream.param.x, stream.param.y, month_sel=month_selec.param.value,
            reg_sel=reg_selec.param.value, per_sel=per_selec.param.value, ens_sel=ens_selec.param.value)
def location(x, y, month_sel, reg_sel, per_sel, ens_sel):

    first_column = pn.pane.Str(f'Click at {x:.2f}, {y:.2f}, month = {month_sel:.0f} , reg = {reg_sel} , per = {per_sel}, ens ={ens_sel}')
    hv_panel[1][0][0].value=per_sel
    hv_panel[1][0][1].value=ens_sel
    hv_panel[1][0][2].value=reg_sel
    ...
    return pn.Column(first_column, second_column, month_selec, reg_selec, per_selec, ens_selec)
